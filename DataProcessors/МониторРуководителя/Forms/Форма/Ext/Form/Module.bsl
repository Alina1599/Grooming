...
Процедура ПриОткрытии(Отказ)
	...
	ЯзыкПоискаЗапроса = "ru";
	СтрокаПоиска = "Бизнес";
	ЗагрузитьНовостиНаСервере(ЯзыкПоискаЗапроса);
КонецПроцедуры
#КонецОбласти
...
Процедура ОбновитьКурс(Команда)
	...
	Если ЯзыкПоиска = 1 Тогда
		ЯзыкПоискаЗапроса = "ru";
	ИначеЕсли ЯзыкПоиска = 2 Тогда
		ЯзыкПоискаЗапроса = "en"; 
	Иначе
		Сообщить("Выберите язык для поиска");
		возврат;
	КонецЕсли;
...
Процедура ЗаполнитьЧисловыеПоказатели()    
	...
	Если ВыборкаЗаписиКлиентовЗавершенные.Следующий() Тогда
		...
		Если ВсегоЗаписей> 0 Тогда
			...
			ЗавершенныхПроцентСтрока = СтрШаблон("Это %1 %% от всех записей", ПроцентЗавершенных); 
		Иначе
...
Процедура ЗаполнитьДиаграммуПоСотрудникам()
	...
	|    Продажи.Сотрудник.Наименование КАК Сотрудник
	|ИЗ
	|    РегистрНакопления.Продажи.Обороты(&ДатаНачала, &ДатаОкончания, Регистратор, ) КАК Продажи
	|ГДЕ
	|    Продажи.СуммаОборот <> 0
	|СГРУППИРОВАТЬ ПО   
	|    Продажи.Сотрудник.Наименование";
...
&НаСервере
Процедура ЗагрузитьНовостиНаСервере(ЯзыкПоискаЗапроса)
	ТопНовости = "";
	...
	Формат(ДатаЗагрузкиНовостей, "ДФ=yyyy-dd-MM"), СокрЛП(СтрокаПоиска)); 
...
&НаКлиенте
Процедура ДатаЗагрузкиНовостейПриИзменении(Элемент)
	Если ЯзыкПоиска = 1 Тогда
		ЯзыкПоискаЗапроса = "ru";
	ИначеЕсли ЯзыкПоиска = 2 Тогда
		ЯзыкПоискаЗапроса = "en";
